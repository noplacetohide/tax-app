...
import { formatMoney, getInvestmentHoldingType } from '@/lib/investmentHelpers'
...
export default function FYInvestment({ fy, title }: InvestmentTableByHoldingTypes) {
    const [investments, setInvestments] = useState([]);
    const [pagination, setPagination] = useState(INIT_PAGINATION_STATE);
    const fetchInvestments = async () => {
        try {
            if (!fy) return;
            const { data } = await api.get(`/api/v1/investments/fy/${fy}`);
